...
import com.example.davidprados.pokedex.adaptadores.MovimientoAdapter;
...
public class FragmentMovimientos extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View vista = inflater.inflate(R.layout.fragment_fragment_movimientos, container,false);

        recycler = vista.findViewById(R.id.recView);
        adapter = new MovimientoAdapter(getContext());
    ...
    private void obtenerMovimientos(){
        ...
        Call<MovimientoRespuesta> call = service.obtenerListaMovimientos(100,0);

        call.enqueue(new Callback<MovimientoRespuesta>() {
            @Override
            public void onResponse(Call<MovimientoRespuesta> call, Response<MovimientoRespuesta> response) {
                if(response.isSuccessful()){
                    MovimientoRespuesta p = response.body();
                    ArrayList<Movimiento> listaMovimientos = p.getResults();

                    adapter.addListaMovimientos(listaMovimientos);
                }
            }

            @Override
            public void onFailure(Call<MovimientoRespuesta> call, Throwable t) {
                Toast toast1 =
